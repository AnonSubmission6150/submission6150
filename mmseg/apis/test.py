    ...
    for i, data in enumerate(data_loader):
        ...
        if show or out_dir:
            ...
            for img, img_meta in zip(imgs, img_metas):
                ...
                result[0] = result[0] - result[0].min()
                result[0] = (result[0] - result[0].min())/result[0].max()
                ...
                heatmap = cv2.applyColorMap(np.uint8(255 * result[0][0]), cv2.COLORMAP_JET)
                cv2.imwrite(out_file, heatmap)
